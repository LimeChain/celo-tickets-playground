...
contract VestingSchedule is UsingRegistry, ReentrancyGuard {
    ...
    constructor (address beneficiary,
        ...
        require(vestingStartTime.add(vestingCliff) > block.timestamp, "Vesting end time is before current time");
    ...
    function revoke(revokeTime) external nonReentrant onlyRevoker {
      ...
      require(getGoldToken().transfer(refundDestination, refundAmount), "Transfer of refund upon revokation failed");
    }
    ...
    function calculateFreeAmountAtTimestamp(uint256 timestamp) private view returns (uint256) {
      ...
      if (timestamp >= vestingScheme.vestingStartTime.add( vestingScheme.vestingPeriods.mul(vestingScheme.vestingPeriodSec) ) || revoked) {
        return totalBalance;
      }
        
      uint256 vestingCurveGradient = (timestamp.sub(vestingScheme.vestingStartTime)).div(vestingScheme.vestingPeriodSec);
      return  ( (currentBalance.mul(vestingCurveGradient)).mul(vestingScheme.vestAmountPerPeriod) ).div(totalBalance);
    }
    ...
    function lockGold(uint256 value) external nonReentrant onlyBeneficiary {
      ...
      require(value <= getGoldToken().balanceOf(address(this)), "Gold amount to lock is greater than the currently vested amount");
     ...
     */
    function authorizeVoteSigner(address signer, uint8 v, bytes32 r, bytes32 s) external nonReentrant onlyBeneficiary {
     ...
     */
    function authorizeValidatorSigner(address signer, uint8 v, bytes32 r, bytes32 s) external nonReentrant onlyBeneficiary {
     ...
     * @dev To be called only by the beneficiary of the vesting. The v,r and s signature should be a signed message by the beneficiary being the authorized address
     */
    function authorizeValidatorSigner(address signer, bytes calldata ecdsaPublicKey, uint8 v, bytes32 r, bytes32 s) external nonReentrant onlyBeneficiary {
     ...
     * @dev To be called only by the beneficiary of the vesting. The v,r and s signature should be a signed message by the beneficiary being the authorized address
     */
    function authorizeAttestationSigner(address signer, uint8 v, bytes32 r, bytes32 s) external nonReentrant onlyBeneficiary {
